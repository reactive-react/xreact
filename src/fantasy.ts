...
//     this.apply = plan
//   }
//   concat<B>(planB: PlanX<E, I, B>) {
//     return intent$ => {
//       let machineB = planB.apply(intent$), machineA = this.apply(intent$)
//       let update$ = streamOps.merge(machineA.update$, machineB.update$)
//       let actions
...
export function map<E extends HKTS, I, A, B>(
  f: (plan: Plan<E, I, A>) => Plan<E, I, B>, fa: FantasyX<E, I, A>
): FantasyX<E, I, B> {
  return fa.map(f)
}

export function lift<E extends HKTS, I, A, B>(
  f: (plan: Plan<E, I, A>) => Plan<E, I, B>
): (fa: FantasyX<E, I, A>) => FantasyX<E, I, B> {
  return fa => fa.map(f)
}

export function lift2<E extends HKTS, I, A, B, C>(
  f: (plan1: Plan<E, I, A>, plan2: Plan<E, I, B>) => Plan<E, I, C>
